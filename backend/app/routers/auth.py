...
@router.post(
    "/register", summary="用户注册", description="创建新用户账号，支持注册后自动登录"
)
    ...
    try:
        async with db.begin():
            ...
            tpl_result = await db.execute(
                select(Project).where(Project.global_demo == 1).limit(1)
            )
            template = tpl_result.scalar_one_or_none()
            if template:
                await copy_project_to_user(
                    db, template.id, user.id, nickname=user.nickname or user.username
                )
            else:
                ...
                db.add(
                    ProjectMember(
                        project_id=default_project.id, user_id=user.id, role="owner"
                    )
                )
...
@router.post(
    "/refresh",
    summary="刷新 Token",
    description="使用 Refresh Token 获取新的 Access Token。支持 Authorization Bearer 头、JSON body 或 Cookie 中的 refresh_token",
)
...
@router.post(
    "/logout", summary="退出登录", description="清除当前登录状态并吊销 Access Token"
)
...
async def change_password(
    ...
    from app.utils.security_audit import SecurityAudit, SecurityEventType
...
@router.post(
    "/forgot-password",
    summary="忘记密码",
    description="请求密码重置码（内部工具直接展示，无需邮件）",
)
@limiter.limit("3/minute")
async def forgot_password(
    ...
    async with db.begin():
        ...
        db.add(
            RefreshToken(
                user_id=user.id,
                token=f"password-reset:{user.id}:{datetime.now(UTC).timestamp()}",
                expires_at=datetime.now(UTC)
...
async def reset_password(
    ...
    if not stored:
        raise_biz(ErrorCodes.AUTH_INVALID_CREDENTIALS, "重置码无效或已过期")
    if datetime.now(UTC) > _to_utc(stored.expires_at):
...
@router.post(
    ...
    summary="管理员重置密码",
    description="管理员验证自身密码后，强制重置指定用户的密码（仅 admin 角色可调用）",
)
@limiter.limit("3/minute")
async def admin_reset_password(
    ...
    async with db.begin():
        ...
        await AuthSvc(db)._revoke_all_user_tokens(req.target_user_id)
    ...
    SecurityAudit.log_event(
        ...
        user_id=current_user.id,
        details={"admin_id": current_user.id, "target_user_id": req.target_user_id},
    )

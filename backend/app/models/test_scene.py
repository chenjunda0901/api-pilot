...
class TestScene(Base, SoftDeleteMixin):
    __test__ = False  # 防止 pytest 误扫描为测试类
    __tablename__ = "test_scenes"
    __table_args__ = (
        Index("ix_test_scenes_project_active", "project_id", "deleted_at"),
    )
    ...
    category_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("scene_categories.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    ...
    var_persist_target: Mapped[str] = mapped_column(
        String(20),
        default="environment",
        nullable=False,
        server_default=text("'environment'"),
    )
    schedule_cron: Mapped[str] = mapped_column(String(100), default="")
    schedule_env_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("environments.id"), nullable=True
    )

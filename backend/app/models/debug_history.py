...
class DebugHistory(Base):
    ...
    api_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True, comment="关联接口ID")
    ...
    request_headers: Mapped[str] = mapped_column(Text, default="[]", comment="请求头JSON数组")
    request_body: Mapped[str] = mapped_column(Text, default="", comment="请求体")
    response_status: Mapped[int] = mapped_column(Integer, nullable=True, comment="响应状态码")
    response_headers: Mapped[str] = mapped_column(Text, default="[]", comment="响应头JSON数组")
